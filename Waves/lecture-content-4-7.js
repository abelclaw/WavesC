...
<p>m d&sup2;x1/dt&sup2; = -2kx1 + kx2</p>
<p>You should think of the first term -2kx1 as the restoring force when mass 1 is moved: it always wants to go back to equilibrium. The second term +kx2 is the force exerted on mass 1 when mass 2 is moved <em>holding everything else fixed</em>. It has a positive sign because if mass 2 moves away, mass 1 wants to follow. If we move mass 3 holding everything else fixed, no force is exerted on mass 1 (it is not directly connected).</p>
<p>Similarly, the middle mass feels forces from both neighbors: m d&sup2;x2/dt&sup2; = -2kx2 + kx1 + kx3, and the third mass is like the first but on the other side: m d&sup2;x3/dt&sup2; = -2kx3 + kx2.</p>
<p>Writing x_vec = (x1_0, x2_0, x3_0) e^{i omega t}, the equations become the matrix eigenvalue problem: -omega^2 x_vec = omega_0^2 M x_vec, where M is the tridiagonal matrix with -2 on the diagonal and 1 on the off-diagonals, and omega_0 = sqrt(k/m).</p>
<details class="derivation-card"><summary>Derive: Three-mass eigenvalues and eigenvectors</summary><div class="derivation-body">
<p>The 3x3 matrix is: [[-2, 1, 0], [1, -2, 1], [0, 1, -2]]. Finding eigenvalues by solving det(M - lambda I) = 0:</p>
...
<p>x_vec(t) = sum_j c_j x_vec_j(t)</p>
<p>where the sum is over normal modes j. Each normal mode x_vec_j(t) oscillates at a single frequency omega_j: x_vec_j(t) = x_vec_j^0 cos(omega_j t), where x_vec_j^0 is the eigenvector (a constant vector describing the shape of the mode).</p>
...
<p><strong>m d&sup2;x_n/dt&sup2; = k x_{n-1} - 2k x_n + k x_{n+1}</strong></p>
<p>The first mass has no mass to its left (attached to a wall): m d&sup2;x_1/dt&sup2; = -2kx_1 + kx_2. The last mass has no mass to its right: m d&sup2;x_N/dt&sup2; = k x_{N-1} - 2k x_N. Putting all of these together with time dependence e^{i omega t} leads to the N x N matrix equation with a <strong>tridiagonal matrix</strong>: -2 on the diagonal, 1 on the off-diagonals, and all other entries zero.</p>`,
...
<p>x_n = B sin(pn) e^{i omega t}</p>
<p>for some value p. The boundary conditions imply that p = (pi / (N+1)) * j for j = 1, 2, 3, ... These p values are called <strong>wavenumbers</strong>. In the continuum limit, we will see that wavenumber p = 2 pi / lambda with lambda the wavelength. In the discrete case, p is dimensionless so it is harder to think of it as related to a wavelength. The fact that wavenumbers are <strong>quantized by the boundary conditions</strong> is extremely important, both classically and in quantum mechanics.</p>`,
    interactive: "n-mass-modes-numerical",
    interactiveCaption: "Normal mode shapes for 6 and 20 coupled masses, showing the sinusoidal envelope emerging",
    mathLinks: ["boundary-conditions"]
  },
  {
    heading: "N Modes: Exact Solution",
    body: `<p>With the numerical solution giving a hint of where to look, let us just solve the system. We want to find vectors x_n that satisfy the equation of motion for the middle masses: d&sup2;x_n/dt&sup2; = omega_0^2 [x_{n-1} - 2x_n + x_{n+1}].</p>
...
<p><strong>Step 3: Simplify.</strong> Dividing both sides by -e^{ipn}: omega^2 = [2 - e^{-ip} - e^{ip}] omega_0^2 = [2 - 2cos(p)] omega_0^2.</p>
<p><strong>Result:</strong> omega(p) = +/- sqrt(2(1 - cos(p))) * omega_0. This is the <strong>dispersion relation</strong>.</p>
</div></details>
...
<p><strong>Check mass 1:</strong> Plugging x_n = B sin(pn) into the equation for mass 1 (m d^2 x1/dt^2 = -2kx1 + kx2), using sin(2p) = 2 sin(p)cos(p), we find it works with the dispersion relation omega^2 = [2 - 2cos(p)] omega_0^2.</p>
<p><strong>Check mass N:</strong> The equation for mass N gives: -B omega^2 sin(Np) = B omega_0^2 [sin((N-1)p) - 2sin(Np)]. After substituting the dispersion relation, canceling the -2sin(Np) terms, and expanding sin(Np - p) using the subtraction formula, we arrive at: 0 = sin((N+1)p).</p>
<p><strong>Quantization:</strong> This is only satisfied if p = (pi/(N+1)) j, for j = 1, 2, 3, ...</p>
<p>This confirms the rigid walls correspond to boundary conditions where masses n = 0 and n = N+1 are held fixed.</p>
</div></details>
<p>Thus the <strong>normal mode frequencies</strong> are: omega^2 = 2(1 - cos(pi j / (N+1))) omega_0^2, for j = 1, 2, 3, ..., N. You can verify this gives the correct answer for N = 3 by plugging in j = 1, 2, 3 and recovering the frequencies we found earlier.</p>
<p>For large N, the lowest frequencies have j much less than N. Using the Taylor expansion cos(x) ~ 1 - x^2/2 for small x, we find omega^2 ~ (pi j / (N+1))^2 omega_0^2, so <strong>omega = omega_0 * p</strong>: the frequency is proportional to the wavenumber. The dispersion relation becomes <strong>linear</strong>. This linearity will be important when we discuss dispersion.</p>`,
  ...
  },
  {
    ...
    body: `<p>Here is a summary of the key results for a large number N of masses connected by springs. For each integer j = 1, 2, 3, ... there is a single normal mode solution. The position of mass n during the oscillation of normal mode j is:</p>
<p><strong>x_n^{(j)}(t) = sin(pi j n / (N+1)) cos(omega_j t + phi_j)</strong></p>
<p>The frequencies are given by: <strong>omega_j = omega_0 sqrt(2(1 - cos(pi j / (N+1))))</strong>. For small j (low modes), this simplifies to omega_j ~ (pi j / (N+1)) omega_0.</p>
<p>An arbitrary solution can be written as a sum over normal modes: x_n(t) = sum_j a_j sin(pi j n / (N+1)) cos(omega_j t + phi_j), for some real constants a_j and phi_j. These solutions all satisfy the boundary conditions x_0(t) = x_{N+1}(t) = 0 (both walls held fixed).</p>
<p>The critical insight: as we increase N, the normal mode shapes become smoother and smoother sine curves, and the discrete system starts to look like a continuous string. This motivates taking the <strong>continuum limit</strong>.</p>`,
  ...
  },
  {
    ...
    body: `<p>We will now take the limit N -> infinity. This will turn our discrete problem into a continuous problem, and our differences into derivatives.</p>
<p>With N masses, we called the displacement of each mass from its equilibrium point x_n. Since all the springs have the same constant, at equilibrium all the masses are a distance Delta_x apart. Let us define a function <strong>A(x, t)</strong> as the <em>amplitude</em> of the displacement from equilibrium at a point x. So A(n Delta_x, t) = x_n(t). To be clear, these displacements are still longitudinal (in the direction of the springs), but we are drawing A(x, t) in the transverse direction for visualization.</p>
...
<p>d^2 A(n Delta_x, t)/dt^2 = (k/m)[A((n+1)Delta_x, t) - 2A(n Delta_x, t) + A((n-1)Delta_x, t)]</p>
<p><strong>Step 2: Recognize the second difference.</strong> The right side can be written as (k/m) Delta_x times the quantity [(A(x+Delta_x,t) - A(x,t))/Delta_x - (A(x,t) - A(x-Delta_x,t))/Delta_x]. This is starting to look like calculus!</p>
<p><strong>Step 3: Take Delta_x -> 0.</strong> The differences become first derivatives: d^2 A/dt^2 = (k/mu)[dA(x,t)/dx - dA(x-Delta_x,t)/dx], where mu = m/Delta_x is the <strong>mass per unit length</strong> (mass density).</p>
<p><strong>Step 4: Define the elastic modulus.</strong> We define E = k Delta_x as the <strong>elastic modulus</strong>. Taking Delta_x -> 0 one more time turns the first derivatives into a second derivative.</p>
<p><strong>Step 5: Define wave speed.</strong> Writing v = sqrt(E/mu), we arrive at the <strong>wave equation</strong>:</p>
<p>d^2 A/dt^2 = v^2 d^2 A/dx^2</p>
</div></details>
<p>The wave equation is one of the most important equations in all of physics: <strong>d&sup2;A/dt&sup2; = v&sup2; d&sup2;A/dx&sup2;</strong>. The wave speed is v = sqrt(E/mu), where E is the elastic modulus (stiffness) and mu is the mass density (inertia). More stiffness means faster waves; more mass means slower waves. This makes physical sense.</p>
...
<p><strong>Right-moving traveling wave:</strong> A(x,t) = cos(kx - omega t) = cos((omega/v)(x - vt)). This solution has the property that A(x, t + Delta_t) = A(x - v Delta_t, t), meaning the amplitude at x in the future is given by the amplitude at a position to the left at the current time. The curve is moving to the right. More generally, A(x,t) = f(x - vt) is a right-moving traveling wave for <em>any</em> function f. Similarly, A(x,t) = f(x + vt) is a <strong>left-moving traveling wave</strong>.</p>
<p><strong>Standing wave:</strong> A(x,t) = cos(kx - omega t) + cos(kx + omega t) = 2cos(kx)cos(omega t). The ratio of amplitudes at any two points stays constant in time: A(x1,t)/A(x2,t) = cos(kx1)/cos(kx2). The shape does not move -- it just oscillates in place.</p>
<p>The key observations: whether a traveling wave or a standing wave is produced <strong>depends on initial conditions</strong>. And standing waves are the sum of a left-moving and right-moving wave. Any traveling wave can be written as a sum over normal modes -- how this is done is the <strong>Fourier decomposition</strong>, which we study next.</p>`,
    ...
    interactiveCaption: "Comparison of traveling waves (f(x - vt) moving right) and standing waves (2cos(kx)cos(omega t) oscillating in place)",
...
<p><strong>Problem 2:</strong> For N = 4 masses, write down the 4x4 tridiagonal matrix. Find the normal mode frequencies using the formula omega^2 = 2(1 - cos(pi j / 5)) omega_0^2 for j = 1, 2, 3, 4. Verify that the j = 1 and j = 4 modes give the slowest and fastest frequencies respectively.</p>
<p><strong>Problem 3:</strong> Show that for the N-mass system, the guess x_n = B sin(pn) e^{i omega t} satisfies the equation of motion for mass 1 (m d^2 x_1/dt^2 = -2kx_1 + kx_2) when omega^2 = 2(1 - cos(p)) omega_0^2. Hint: use sin(2p) = 2sin(p)cos(p).</p>
<p><strong>Problem 4:</strong> In the continuum limit derivation, explain physically why the elastic modulus is defined as E = k Delta_x rather than just k. What happens to E as we take Delta_x -> 0 with k fixed? Why must k scale as 1/Delta_x for E to remain finite?</p>
<p><strong>Problem 5:</strong> Verify that A(x, t) = f(x - vt) satisfies the wave equation for any twice-differentiable function f. Compute d^2 f/dt^2 and d^2 f/dx^2 using the chain rule.</p>
<p><strong>Problem 6:</strong> Show that a standing wave 2cos(kx)cos(omega t) can be written as the sum of a left-moving and right-moving traveling wave. Use the identity cos(A) + cos(B) = 2cos((A+B)/2)cos((A-B)/2).</p>
<p><strong>Problem 7:</strong> A slinky has 100 coils, each of mass 2g, with spring constant k = 1 N/m between adjacent coils. (a) Find omega_0. (b) Find the frequency of the lowest normal mode. (c) Find the frequency of the 10th normal mode. (d) For the 10th mode, compare the exact frequency with the approximate formula omega ~ (pi j / (N+1)) omega_0.</p>`,
    interactive: null,
    interactiveCaption: null,
    mathLinks: []
  }
...
"5": [
  {
    ...
    body: `<p>When N oscillators are strung together in a series, the amplitude of that string can be described by a function A(x, t) which satisfies the <strong>wave equation</strong>: [d^2/dt^2 - v^2 d^2/dx^2] A(x, t) = 0. We saw that electromagnetic fields satisfy this same equation with v = c the speed of light.</p>
<p>We found normal mode solutions of the form A(x, t) = A_0 cos((omega/v)(x +/- vt) + phi), which are <strong>traveling waves</strong>. Solutions of the form A(x, t) = A_0 cos(kx) cos(omega t) with omega^2 = v^2 k^2 are called <strong>standing waves</strong>. Whether traveling waves or standing waves are relevant depends on the boundary conditions.</p>
...
<p>(Ok, we have not really proven it -- we just assumed the result from linear algebra about a finite system applies also in the continuum limit. The actual proof requires certain properties about the smoothness of f(x) to hold. But we are physicists not mathematicians, so let us just say we proved it.)</p>`,
  ...
  },
  {
    ...
    body: `<p><strong>Fourier's theorem</strong> states that any square-integrable function f(x) which is periodic on the interval 0 < x <= L (meaning f(x + L) = f(x)) can be written as:</p>
<p><strong>f(x) = a_0 + sum_{n=1}^{infinity} a_n cos(2 pi n x / L) + sum_{n=1}^{infinity} b_n sin(2 pi n x / L)</strong></p>
<p>with coefficients: <strong>a_0 = (1/L) integral_0^L f(x) dx</strong>, <strong>a_n = (2/L) integral_0^L f(x) cos(2 pi n x / L) dx</strong>, and <strong>b_n = (2/L) integral_0^L f(x) sin(2 pi n x / L) dx</strong>.</p>
<p>This decomposition is known as a <strong>Fourier series</strong>. Fourier series are useful for periodic functions or functions on a fixed interval L (like a string). One can do a similar analysis for non-periodic functions or functions on an infinite interval (L -> infinity) in which case the decomposition is known as a <strong>Fourier transform</strong>. We will study Fourier series first.</p>
...
<p><strong>For a_0:</strong> We just integrate f(x) over the interval. Since cos(2 pi n x / L) goes through n complete cycles as x goes from 0 to L, its integral vanishes: integral_0^L cos(2 pi n x / L) dx = 0 for n > 0. Similarly for sine. Thus integral_0^L f(x) dx = a_0 * L, giving a_0 = (1/L) integral_0^L f(x) dx.</p>
<p><strong>For a_n:</strong> Multiply f(x) by cos(2 pi n x / L) and integrate. We need the cosine product integral. Using the sum formula: cos(2 pi m x / L) cos(2 pi n x / L) = (1/2) cos((n+m) 2 pi x / L) + (1/2) cos((n-m) 2 pi x / L).</p>
<p>Both terms vanish when integrated over a complete period, <em>unless</em> n - m = 0. So: integral_0^L cos(2 pi m x / L) cos(2 pi n x / L) dx = (L/2) delta_{mn}, where delta_{mn} is the <strong>Kronecker delta</strong> (1 if m = n, 0 otherwise).</p>
<p>Similarly: integral_0^L cos(2 pi m x / L) sin(2 pi n x / L) dx = 0 (always), and integral_0^L sin(2 pi m x / L) sin(2 pi n x / L) dx = (L/2) delta_{mn}.</p>
<p>So: integral_0^L f(x) cos(2 pi n x / L) dx = a_0 * 0 + sum_m a_m (L/2) delta_{mn} + sum_m b_m * 0 = (L/2) a_n.</p>
<p>Solving gives a_n = (2/L) integral_0^L f(x) cos(2 pi n x / L) dx, exactly as claimed. The derivation for b_n is identical with cosine replaced by sine.</p>
</div></details>
<p>The orthogonality relations are the heart of Fourier analysis. They are the continuous analog of the orthogonality of eigenvectors in linear algebra. We will see this connection made precise in the section on orthogonal functions.</p>`,
  ...
  },
  {
    ...
    body: `<p>Let us find the Fourier series for the <strong>sawtooth function</strong>, which equals f(x) = x on the interval 0 < x <= 1 and repeats periodically. This function is clearly periodic with L = 1.</p>
<details class="derivation-card"><summary>Derive: Fourier series of the sawtooth function</summary><div class="derivation-body">
<p><strong>a_0:</strong> a_0 = integral_0^1 x dx = 1/2.</p>
<p><strong>a_n:</strong> a_n = 2 integral_0^1 x cos(2 pi n x) dx. Integration by parts: a_n = 2 [x/(2 pi n) sin(2 pi n x)]_0^1 - 2 integral_0^1 sin(2 pi n x) / (2 pi n) dx. The boundary term vanishes because sin(2 pi n) = 0, and the integral also gives zero. So <strong>a_n = 0</strong> for all n.</p>
<p><strong>b_n:</strong> b_n = 2 integral_0^1 x sin(2 pi n x) dx. Integration by parts: b_n = -2 [x/(2 pi n) cos(2 pi n x)]_0^1 + 2 integral_0^1 cos(2 pi n x) / (2 pi n) dx = -1/(pi n) + 0 = <strong>-1/(pi n)</strong>.</p>
</div></details>
<p>Thus the Fourier series of the sawtooth function is: <strong>f(x) = 1/2 + sum_{n=1}^{infinity} (-1/(pi n)) sin(2 pi n x)</strong>.</p>
<p>How well does this work? With just one term in the sum, we get f ~ 1/2 (a constant -- not great). With 2 terms: f ~ 1/2 - (1/pi) sin(2 pi x) -- starting to tilt. With 3 terms the shape is recognizable. Already at 10 modes we find excellent agreement with the sawtooth. By 100 modes the approximation is essentially perfect everywhere except at the discontinuity (where you see a small overshoot known as the <strong>Gibbs phenomenon</strong>).</p>`,
...
<p>We write the full time-dependent solution as: A(x, t) = sum_{n=0}^{infinity} [a_n cos(2n pi x / L) cos(omega_n t) + b_n sin(2n pi x / L) cos(omega_n t)], where omega_n = (2n pi / L) v is determined by the dispersion relation. Here v is the speed of sound in the string. We could also have included sin(omega_n t) terms, but since the string starts at rest (dA/dt = 0 at t = 0), those coefficients must all vanish.</p>
<p>At t = 0, the amplitude is just the Fourier decomposition of the pluck shape. If we approximate the pluck as the sawtooth function, then a_n = 0 and b_n = -1/(pi n). So setting L = 1: <strong>A(x, t) = sum_{n=1}^{infinity} (-1/(pi n)) sin(2 pi n x) cos(2 pi n v t)</strong>. This gives the motion of the string for all time!</p>
...
<p><strong>f(x) = sum_{n=-infinity}^{infinity} c_n e^{i n x 2 pi / L}</strong></p>
<p>where <strong>c_n = (1/L) integral_0^L f(x) e^{-i n x 2 pi / L} dx</strong>.</p>
...
<p>Substitute the series into the integral: integral_0^L f(x) e^{-inx 2pi/L} dx = sum_m c_m integral_0^L e^{i(m-n)x 2pi/L} dx.</p>
<p>If n != m, then: integral_0^L e^{i(m-n)x 2pi/L} dx = (L / (2pi)) * (1/(m-n)) * [e^{i 2pi(m-n)} - 1] = 0, since e^{i 2pi k} = 1 for any integer k.</p>
<p>If m = n, the integral is just integral_0^L dx = L.</p>
<p>Thus integral_0^L e^{i(m-n)x 2pi/L} dx = L delta_{mn}, giving integral_0^L f(x) e^{-inx 2pi/L} dx = L c_n, confirming the formula.</p>
</div></details>
<p>If f(x) is real, then c_n and c_{-n} are complex conjugates, and we recover: a_n = Re(c_n + c_{-n}) and b_n = Im(c_{-n} + c_n). The exponential series contains all the information in both the sine and cosine series in a single, efficient form. The exponential notation is almost always preferred in advanced physics.</p>
<span class="inline-math-link" data-math="complex-exponentials">Complex Exponentials &rarr;</span>`,
  ...
  },
  {
    ...
    body: `<p>In verifying Fourier's theorem, we found integral equations like (1/L) integral_0^L e^{i(m-n)x 2pi/L} dx = delta_{mn}. These are examples of <strong>orthogonal functions</strong>. The integral is a type of <strong>inner product</strong>, exactly analogous to the dot product among vectors.</p>
<p>We can define the inner product of two functions as: <strong>&lt;f|g&gt; = (1/2pi) integral_0^{2pi} f*(x) g(x) dx</strong>, where f*(x) is the complex conjugate. For example, &lt;e^{imx} | e^{inx}&gt; = delta_{mn}. This is the analog of &lt;x_i | x_j&gt; = delta_{ij} for unit vectors.</p>
<p>When a set of functions satisfy &lt;f_i | f_j&gt; = delta_{ij}, we say they are <strong>orthonormal</strong>. The "ortho" part means orthogonal (&lt;f_i | f_j&gt; = 0 for i != j), and the "normal" part means normalized (&lt;f_i | f_i&gt; = 1). If any function can be written as a linear combination of the f_i, we say the set is <strong>complete</strong>. Then f(x) = sum_i a_i f_i(x), and we can extract coefficients via a_i = &lt;f | f_i&gt;. This is exactly what we do with the Fourier decomposition, and it is also what we do with vectors: v_vec = sum_i c_i x_hat_i, and c_i = v_vec dot x_hat_i.</p>
<p>Other important examples of orthonormal function bases include: <strong>Bessel functions</strong> J_n(x), which come up in 2D problems; <strong>Legendre polynomials</strong> P_n(x), which come up in spherical problems (and you will study to death in quantum mechanics); and <strong>Hermite polynomials</strong> H_n(x), which play a critical role in the quantum harmonic oscillator. Each family has its own inner product and its own completeness relation.</p>`,
  ...
  {
    heading: "Problems",
    body: `<p><strong>Problem 1:</strong> Compute the Fourier series for the square wave: f(x) = 1 for 0 < x < L/2 and f(x) = -1 for L/2 < x < L. Show that only odd harmonics (b_1, b_3, b_5, ...) are nonzero, and find their values.</p>
<p><strong>Problem 2:</strong> Verify the orthogonality relation integral_0^L sin(2 pi m x / L) sin(2 pi n x / L) dx = (L/2) delta_{mn} by direct computation. Hint: use the product-to-sum identity sin(A)sin(B) = (1/2)[cos(A-B) - cos(A+B)].</p>
<p><strong>Problem 3:</strong> A string of length L = 1m is plucked in the middle, creating a triangular shape: A(x, 0) = 2x for 0 < x < 1/2 and A(x, 0) = 2(1-x) for 1/2 < x < 1. Find the Fourier sine series coefficients b_n. Show that the even harmonics vanish and explain why physically.</p>
...
<p><strong>Problem 7:</strong> Show that the Bessel functions J_0(x) and J_1(x) are orthogonal with respect to the inner product integral_0^1 x J_m(x) J_n(x) dx. You may use the integral table result or verify numerically.</p>`,
    interactive: null,
    interactiveCaption: null,
    mathLinks: []
  }
...
"6": [
  {
    ...
    body: `<p>In Lecture 4, we derived the wave equation for two systems. First, by stringing together masses and springs and taking the continuum limit, we found the wave equation [d^2/dt^2 - v^2 d^2/dx^2] A(x, t) = 0, where A(x, t) is the displacement from equilibrium of the mass at position x. These are <strong>longitudinal waves</strong>. The wave speed is v = sqrt(E/mu), where E is the elastic modulus and mu is the density per unit length.</p>
...
<p><strong>Step 1: Force from the left.</strong> Consider the downward component of force pulling on the mass at x from the mass to the left (at x - Delta_x). Drawing a triangle, the force is F_down,left = T sin(theta) = T Delta_A / sqrt(Delta_A^2 + Delta_x^2). If the system is close to equilibrium, the slope is small (Delta_A << Delta_x), so we approximate sqrt(Delta_A^2 + Delta_x^2) ~ Delta_x, giving:</p>
<p>F_down,left = T (A(x) - A(x - Delta_x)) / Delta_x = T dA/dx</p>
<p><strong>Step 2: Force from the right.</strong> Similarly: F_down,right = T (A(x) - A(x + Delta_x)) / Delta_x = -T dA(x + Delta_x)/dx.</p>
<p><strong>Step 3: Total force.</strong> F_total = -T [dA(x + Delta_x)/dx - dA(x)/dx].</p>
<p><strong>Step 4: Apply F = ma.</strong> The downward acceleration is a = -d^2 A/dt^2, so F_total = -mu Delta_x d^2 A/dt^2, where mu = m/Delta_x is the mass per unit length.</p>
<p><strong>Step 5: Combine.</strong> d^2 A/dt^2 = (T/mu) [dA(x+Delta_x)/dx - dA(x)/dx] / Delta_x = (T/mu) d^2 A/dx^2.</p>
</div></details>
<p>The result is the wave equation: <strong>[d^2/dt^2 - v^2 d^2/dx^2] A(x, t) = 0</strong> with wave speed <strong>v = sqrt(T/mu)</strong>.</p>
...
<p><strong>v = sqrt((dp/drho)_S)</strong></p>
<details class="derivation-card"><summary>Derive: Speed of sound using the adiabatic index</summary><div class="derivation-body">
<p>From thermodynamics, (dp/drho)_S = gamma * p/rho, where gamma = C_P / C_V is the ratio of specific heats. A more useful formula is gamma = (f + 2) / f, where f is the number of degrees of freedom.</p>
<p>For a <strong>monatomic gas</strong> (like Argon), only translations matter: f = 3, so gamma = 5/3 = 1.67.</p>
<p>For a <strong>diatomic gas</strong> (like N_2 or O_2, which is mostly what air is), both atoms can move but rotation around the bond axis leaves the molecule unchanged, so f = 5 (3 translations + 1 rotation + 1 vibration). Thus gamma = 7/5 = 1.4.</p>
</div></details>
...
<p>You may recall from chemistry that the root-mean-square velocity of a gas is v_rms = sqrt(3RT/m). So c_s = sqrt(gamma/3) v_rms. The speed of sound is proportional to, but not greater than, the speed of the molecules in the gas. This makes sense -- how could sound travel faster than the molecules transmitting it?</p>`,
    interactive: null,
    interactiveCaption: null,
...
<p>A(x, t) = A_0 sin(kx + phi_1) sin(omega t + phi_2)</p>
...
<p><strong>Free (Neumann) boundary condition:</strong> If one end is free, what condition applies? Going back to the force analysis: if there is no string to the right, then F_right = 0 and the only force on the end mass is from the left. Taking Delta_x -> 0, this requires <strong>dA(L, t)/dx = 0</strong> at the free end.</p>`,
  ...
  },
  {
    ...
    body: `<p><strong>Both ends fixed:</strong> We need sin(kL) = 0, which means k = n pi / L for n = 1, 2, 3, ... The allowed frequencies are:</p>
<p><strong>omega_n = v (pi/L) n, n = 1, 2, 3, ...</strong> (both ends fixed)</p>
<p>This gives harmonics at nu, 2nu, 3nu, 4nu, ... All integer multiples are present.</p>
<p><strong>One end fixed, one end free:</strong> Using A(x, t) = A_0 sin(kx) sin(omega t + phi) and the Neumann condition dA/dx = 0 at x = L, we need cos(kL) = 0. This means kL = (n + 1/2) pi, giving:</p>
<p><strong>omega_n = v (n + 1/2) pi / L, n = 0, 1, 2, 3, ...</strong> (one fixed, one free)</p>
<p>The lowest frequency is nu_0 = v/(4L). The next is 3 nu_0, then 5 nu_0, then 7 nu_0. <strong>The even harmonics are missing!</strong> This has dramatic consequences for instruments like the trumpet and the clarinet.</p>
<p><strong>Both ends free:</strong> We need A(x, t) = A_0 cos(kx) sin(omega t + phi) and sin(kL) = 0, giving omega_n = v n pi / L for n = 0, 1, 2, 3, ... The only difference from both-fixed is that n = 0 is allowed, but that mode is just a constant (not physically interesting).</p>
<p>Here is the summary table of harmonics for the fundamental frequency nu:</p>
<table><tr><th>Boundary</th><th>Lowest</th><th>Next</th><th>Second</th><th>Third</th></tr>
<tr><td>Both fixed</td><td>nu</td><td>2nu</td><td>3nu</td><td>4nu</td></tr>
<tr><td>One fixed, one free</td><td>nu</td><td>3nu</td><td>5nu</td><td>7nu</td></tr>
<tr><td>Both free</td><td>nu</td><td>2nu</td><td>3nu</td><td>4nu</td></tr></table>
...
<p>We use omega = sqrt(k_spring / m). The spring constant comes from F = -k Delta_x. For pressure, F = A * dp where A is the cross-sectional area of the neck.</p>
<p>Since rho = m/V, we get drho = -(m/V^2) dV = -rho (dV/V). Using dp/drho = gamma p/rho for sound waves: dp = -gamma (p/V) dV.</p>
<p>Since dV = A Delta_x: F = A * dp = -gamma A^2 (p/V) Delta_x. So k_spring = gamma A^2 p/V = A^2 c_s^2 rho / V.</p>
<p>The mass is the air in the neck: m = rho A L. Therefore: omega = sqrt(A^2 c_s^2 rho / (V rho A L)) = c_s sqrt(A / (VL)).</p>
</div></details>
<p>Helmholtz resonators resonate at a single frequency: <strong>nu = (c_s / 2 pi) sqrt(A / (VL))</strong>, where A is the area of the opening, L is the length of the neck, and V is the volume of the cavity. For example, a 10 cm wide jar with a 1 cm^2 opening and 1 cm long neck containing 1 liter of air gives nu ~ 172 Hz, with a wavelength of about 2 m -- much larger than the resonator itself.</p>
...
<p><strong>Problem 2:</strong> Derive the wave equation for transverse oscillations from scratch. Start with a string element of length Delta_x and tension T. Carefully identify the vertical components of tension at each end, apply Newton's second law, and take Delta_x -> 0.</p>
...
<p><strong>Problem 4:</strong> Calculate the speed of sound in helium at room temperature (T = 293 K). Helium is monatomic with molecular weight m = 4 g/mol. Compare to the speed of sound in air (m ~ 29 g/mol, gamma = 1.4).</p>
...
<p>Thus there are two reasons sounds appear unmusical: <strong>too many frequencies present at once</strong>, and <strong>beating at frequencies we can consciously process</strong> (roughly below 20-30 Hz). This is a physics class, not a biology class, so we will not try to explain <em>why</em> these facts hold. We merely observe that whenever either criterion is satisfied, sounds appear unmusical.</p>`,
  ...
  },
  {
    ...
    body: `<p>If we play a pure 300 Hz sine wave with a pure 580 Hz sine wave, it does not sound bad: cos(300 * 2pi t) + cos(580 * 2pi t) = 2 cos(140 * 2pi t) cos(440 * 2pi t). The beat frequency 2 * 140 Hz = 280 Hz is too high to be harsh -- it is just a note. But if we played 300 Hz and 580 Hz on an <em>actual instrument</em>, it would sound horrible.</p>
<p>Why? Because real instruments excite harmonics. For a string plucked near the end, the Fourier coefficients scale like 1/n. The fundamental (n = 1) has only twice the amplitude of the first harmonic (n = 2). Playing 580 Hz alongside a plucked 300 Hz string gives: f(t) = cos(580T) + cos(300T) + (1/2)cos(600T) + ... . The 580 Hz and 600 Hz terms combine to produce beating at 2 * 10 = 20 Hz, which is audible and jarring. There is beating between the external 580 Hz note and the first harmonic of the plucked string.</p>
...
<p>The key points are: on a real instrument, there will be unmusical beating whenever an integer multiple of one harmonic is <strong>close but not equal</strong> to an integer multiple of another harmonic. Conversely, the most <strong>consonant</strong> notes will have some harmonics which <em>exactly</em> agree.</p>`,
  ...
  },
  {
    ...
    body: `<p>Let us start with middle C (C4, frequency nu_0 = 261 Hz). Which notes sound good alongside C4? The 261 Hz note has harmonics at nu_0, 2 nu_0, 3 nu_0, 4 nu_0, etc: 261 Hz, 522 Hz, 783 Hz, 1044 Hz, 1305 Hz, ... Playing any of those frequencies alongside C4 will sound harmonic because they exactly match an existing harmonic.</p>
<p>The note at 2 nu_0 is one <strong>octave</strong> up (the 1st harmonic) = C5. Are there more harmonious notes between? Yes. Consider the note at nu_5 = 391 Hz. Its second harmonic is 2 * 391 = 783 Hz, which matches the 3rd harmonic of C4. We call nu_5 = (3/2) nu_0 the <strong>perfect fifth</strong> (G4).</p>
<p>Similarly, nu_4 = 348 Hz has its 3rd harmonic matching the 4th harmonic of C4. We call nu_4 = (4/3) nu_0 the <strong>perfect fourth</strong> (F4).</p>
<p>Any rational number ratio of frequencies will be consonant. Many of these ratios have standard names:</p>
<table>
<tr><th>Ratio nu/nu_0</th><th>1</th><th>2</th><th>3/2</th><th>4/3</th><th>5/4</th><th>6/5</th><th>5/3</th><th>8/5</th></tr>
...
<tr><th>nu/nu_0</th><td>1</td><td>9/8</td><td>5/4</td><td>4/3</td><td>3/2</td><td>5/3</td><td>15/8</td><td>2</td></tr>
...
<p>To see the problem concretely: the just intonation scale defines D as (9/8) nu_0. The perfect 5th of D should be at (3/2) * (9/8) = 27/16 = 1.688 times nu_0. This note falls between A and B -- it is not exactly any note in the key of C scale. To play any note in any key perfectly, you would need an enormous number of available notes.</p>
<p>On a violin (which has no frets), a skilled player can produce any frequency and play the just intonation scale in any key. On a piano or guitar, the notes are built in. You can sometimes tune to a different key, but there are a finite number of notes. <strong>It is impossible to have an instrument with a finite number of notes be capable of playing the most harmonic notes in every key.</strong></p>`,
  ...
  },
  {
    ...
    body: `<p>One way to approximate an all-key scale is by choosing the notes to be related by powers of 3/2 (perfect fifths) and octaves (factors of 2). Starting with C: (3/2) nu_0 is the fifth (G4). Then (3/2)^2 nu_0 is the fifth of the fifth (D5). Then (3/2)^3 nu_0 is the fifth of D5 (A6), and so on. We can bring any power of 3/2 back to the original octave by dividing by appropriate powers of 2. For example, D5 = (9/4) nu_0, so D4 = (9/8) nu_0.</p>
<table>
<tr><th>Note</th><td>C</td><td>D</td><td>E</td><td>F</td><td>G</td><td>A</td><td>B</td><td>C</td></tr>
<tr><th>nu/nu_0</th><td>1</td><td>9/8</td><td>81/64</td><td>4/3</td><td>3/2</td><td>27/16</td><td>243/128</td><td>2</td></tr>
...
<p>The advantage of Pythagorean tuning is that the perfect fifth and perfect fourth of <em>every</em> note is included in the scale (since all notes are related by fifths).</p>`,
  ...
  },
  {
    ...
    body: `<p>An interesting feature of the Pythagorean scale is that the 12th fifth is very close to 8 octaves: <strong>(3/2)^12 = 129.748 ~ 128 = 2^7</strong>. Thus we can make a compromise: declare that 12 steps around the <strong>circle of fifths</strong> gets you back to the note you started at. We simply relate all notes by powers of 2^{1/12}. Each half-step gives another factor of 2^{1/12}.</p>
<table>
<tr><th>Note</th><td>C</td><td>C#</td><td>D</td><td>D#</td><td>E</td><td>F</td><td>F#</td><td>G</td><td>G#</td><td>A</td><td>A#</td><td>B</td><td>C</td></tr>
<tr><th>nu/nu_0</th><td>1</td><td>2^{1/12}</td><td>2^{2/12}</td><td>2^{3/12}</td><td>2^{4/12}</td><td>2^{5/12}</td><td>2^{6/12}</td><td>2^{7/12}</td><td>2^{8/12}</td><td>2^{9/12}</td><td>2^{10/12}</td><td>2^{11/12}</td><td>2</td></tr>
...
<p><strong>Problem 3:</strong> In the just intonation scale, the major third of C is E at 5/4 nu_0 and the major third of E is G# at (5/4)^2 nu_0. Compare (5/4)^3 to 2. This shows that three major thirds do not make an octave in just intonation. By how many cents is the discrepancy? (A cent is 1/1200 of an octave, or a factor of 2^{1/1200}.)</p>
<p><strong>Problem 4:</strong> A flute playing G4 (392 Hz) has harmonics at 392, 784, 1176, 1568, ... Hz. A flute playing C5 (523 Hz) has harmonics at 523, 1046, 1569, ... Hz. Identify which harmonics nearly coincide. What is the beat frequency between the closest pair? Does this explain why G and C sound consonant?</p>
...
<p><strong>Problem 6:</strong> Verify that (3/2)^12 = 129.746... and compare to 2^7 = 128. Compute the Pythagorean comma as the ratio (3/2)^12 / 2^7. Express this in cents.</p>
