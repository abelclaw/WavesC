...
function initEigenvalueSolver() {
  ...
  // Draw a waveform in a plot box
  function drawWaveform(plotX, plotY, plotW, plotH, fn, tMax, tNow, color, label) {
    // Background box
    ...
    for (let px = 0; px <= plotW; px += 1) {
      const tVal = (px / plotW) * tMax;
      const val = fn(tVal);
      const py = midY - val * amp;
      if (first) { ctx.moveTo(plotX + px, py); first = false; }
      else ctx.lineTo(plotX + px, py);
    }
    ctx.stroke();

    // Time marker
    const markerX = plotX + (tNow / tMax) * plotW;
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 1; ctx.globalAlpha = 0.4;
    ctx.beginPath(); ctx.moveTo(markerX, plotY); ctx.lineTo(markerX, plotY + plotH); ctx.stroke();
    ctx.globalAlpha = 1;

    // Dot at current value
    const nowVal = fn(tNow);
    ctx.fillStyle = color;
    ctx.beginPath(); ctx.arc(markerX, midY - nowVal * amp, 3.5, 0, 2 * Math.PI); ctx.fill();

    // Label
    ctx.fillStyle = color; ctx.font = 'bold 11px system-ui, sans-serif'; ctx.textAlign = 'left';
    fillTextSub(ctx, label, plotX + 4, plotY + 12);
  }
  ...
  function draw() {
    ...
    // === TOP: Animated spring-mass system ===
    const mechY = 32, mechH = 38;
    const mechL = W * 0.15, mechR = W * 0.85;
    const mechW = mechR - mechL;
    const eq1 = mechL + mechW * 0.33, eq2 = mechL + mechW * 0.67;
    const massR = 10;
    const disp = 22; // pixels per unit displacement

    const x1Now = x1(tNow), x2Now = x2(tNow);
    const m1x = eq1 + x1Now * disp, m2x = eq2 + x2Now * disp;

    // Walls
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(mechL, mechY - 14); ctx.lineTo(mechL, mechY + 14); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(mechR, mechY - 14); ctx.lineTo(mechR, mechY + 14); ctx.stroke();
    // Hatch marks
    ctx.lineWidth = 1;
    for (let i = -2; i <= 2; i++) {
      const hy = mechY + i * 5;
      ctx.beginPath(); ctx.moveTo(mechL - 5, hy + 3); ctx.lineTo(mechL, hy - 3); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(mechR, hy - 3); ctx.lineTo(mechR + 5, hy + 3); ctx.stroke();
    }

    // Springs
    ctx.strokeStyle = WCOLORS.textDim; ctx.lineWidth = 1.2;
    drawSpringHE(mechL + 3, m1x - massR, mechY, 7, 6);
    ctx.strokeStyle = WCOLORS.amber; ctx.lineWidth = 1.2;
    drawSpringHE(m1x + massR, m2x - massR, mechY, 5, 6);
    ctx.strokeStyle = WCOLORS.textDim; ctx.lineWidth = 1.2;
    drawSpringHE(m2x + massR, mechR - 3, mechY, 7, 6);

    // Masses
    ctx.fillStyle = WCOLORS.teal;
    ctx.beginPath(); ctx.arc(m1x, mechY, massR, 0, 2 * Math.PI); ctx.fill();
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(m1x, mechY, massR, 0, 2 * Math.PI); ctx.stroke();
    ctx.fillStyle = WCOLORS.red;
    ctx.beginPath(); ctx.arc(m2x, mechY, massR, 0, 2 * Math.PI); ctx.fill();
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(m2x, mechY, massR, 0, 2 * Math.PI); ctx.stroke();

    // Mass labels
    ctx.fillStyle = '#fff'; ctx.font = 'bold 10px system-ui, sans-serif'; ctx.textAlign = 'center';
    ctx.fillText('1', m1x, mechY + 4);
    ctx.fillText('2', m2x, mechY + 4);

    // Spring labels
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '10px system-ui, sans-serif'; ctx.textAlign = 'center';
    ctx.fillText('k', (mechL + m1x) / 2, mechY - 16);
    ctx.fillStyle = WCOLORS.amber;
    ctx.fillText('\u03BA', (m1x + m2x) / 2, mechY - 16);
    ctx.fillStyle = WCOLORS.textDim;
    ctx.fillText('k', (m2x + mechR) / 2, mechY - 16);

    // === BOTTOM: Four waveform plots (2×2 grid) ===
    const plotTop = mechY + mechH + 8;
    const plotBot = H - 8;
    const totalPlotH = plotBot - plotTop;
    const rowH = (totalPlotH - 10) / 2; // gap between rows
    const halfW = (W - 30) / 2; // left half and right half

    const leftX = 10, rightX = W / 2 + 8;
    const row1Y = plotTop, row2Y = plotTop + rowH + 10;

    // Column headers
    ctx.fillStyle = WCOLORS.text; ctx.font = 'bold 12px system-ui, sans-serif'; ctx.textAlign = 'center';
    ctx.fillText('Original basis (coupled)', leftX + halfW / 2, row1Y - 2);
    ctx.fillText('Eigenbasis (decoupled)', rightX + halfW / 2, row1Y - 2);

    // Left column: x₁(t) and x₂(t) — messy
    drawWaveform(leftX, row1Y + 4, halfW, rowH - 4, x1, tMax, tNow, WCOLORS.teal, 'x\u2081(t)');
    drawWaveform(leftX, row2Y, halfW, rowH - 4, x2, tMax, tNow, WCOLORS.red, 'x\u2082(t)');

    // Right column: ξs(t) and ξa(t) — clean single frequencies
    drawWaveform(rightX, row1Y + 4, halfW, rowH - 4, xiS, tMax, tNow, WCOLORS.teal, '\u03BE_s(t)');
    drawWaveform(rightX, row2Y, halfW, rowH - 4, xiA, tMax, tNow, WCOLORS.red, '\u03BE_a(t)');

    // Frequency annotations on right-side plots
    ctx.font = '10px system-ui, sans-serif'; ctx.textAlign = 'right';
    ctx.fillStyle = WCOLORS.teal;
    ctx.fillText('\u03C9 = ' + omegaS.toFixed(2), rightX + halfW - 4, row1Y + 23);
    ctx.fillStyle = WCOLORS.red;
    ...
    const arrowL = leftX + halfW + 4, arrowR = rightX - 4;
    const arrowMid = (arrowL + arrowR) / 2;
    ctx.strokeStyle = WCOLORS.amber; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(arrowL, arrowY); ctx.lineTo(arrowR, arrowY); ctx.stroke();
    ctx.fillStyle = WCOLORS.amber;
    ctx.beginPath();
    ctx.moveTo(arrowR, arrowY);
    ctx.lineTo(arrowR - 5, arrowY - 3);
    ctx.lineTo(arrowR - 5, arrowY + 3);
    ctx.closePath(); ctx.fill();
    // Label on arrow
...
function initEmSpectrum() {
  ...
  // Visible light: 380nm–780nm => log10(380e-9)=-6.42, log10(780e-9)=-6.11
  const bands = [
  ...
  var logLambdaMin = -16, logLambdaMax = 4;
  var specLeft = 50, specRight = W - 14, specW = specRight - specLeft;
  var freqAxisY = 38, barTop = 58, barH = 44;
  var lambdaAxisY = barTop + barH + 12;
  var iconRowTop = lambdaAxisY + 28, iconRowH = 80;
  var infoY = iconRowTop + iconRowH + 6;

  var markerLogLambda = -6.3, dragging = false;
  ...
  // Attempt to produce a bright, vivid rainbow. We use a simple piecewise
  // linear map that saturates each channel fully so the bar never looks dark.
  function wavelengthToRGB(nm) {
    ...
    // slight edge fade so it doesn't clip harshly
    var f = 1;
    if (nm < 420)      f = 0.4 + 0.6 * (nm - 380) / 40;
    else if (nm > 700) f = 0.4 + 0.6 * (780 - nm) / 80;
    return 'rgb(' + Math.round(r*f*255) + ',' + Math.round(g*f*255) + ',' + Math.round(b*f*255) + ')';
  }

  function getBandAt(logL) {
    for (var i = 0; i < bands.length; i++) {
      if (logL >= bands[i].logMin && logL <= bands[i].logMax) return bands[i];
    }
    return null;
  }
  ...
  function drawRadioTower(cx, cy, s) {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy - s * 1.1); ctx.lineTo(cx - s * 0.45, cy + s * 0.8); ctx.lineTo(cx + s * 0.45, cy + s * 0.8);
    ctx.closePath(); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx, cy - s * 1.1); ctx.lineTo(cx, cy - s * 1.5); ctx.stroke();
    ctx.strokeStyle = WCOLORS.teal; ctx.lineWidth = 1;
    for (var i = 1; i <= 3; i++) {
      var r = s * 0.3 * i;
      ctx.beginPath(); ctx.arc(cx, cy - s * 1.5, r, -Math.PI * 0.7, -Math.PI * 0.3); ctx.stroke();
      ctx.beginPath(); ctx.arc(cx, cy - s * 1.5, r, Math.PI * 0.3, Math.PI * 0.7); ctx.stroke();
    }
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('Radio tower', cx, cy + s * 0.8 + 11);
  }

  function drawMicrowaveOven(cx, cy, s) {
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 1.5;
    var bx = cx - s * 0.9, by = cy - s * 0.55, bw = s * 1.8, bh = s * 1.1;
    ctx.strokeRect(bx, by, bw, bh);
    ctx.strokeRect(bx + 2, by + 2, bw * 0.58, bh - 4);
    ctx.beginPath(); ctx.moveTo(bx + bw * 0.66, by + bh * 0.3); ctx.lineTo(bx + bw * 0.66, by + bh * 0.7); ctx.stroke();
    ctx.fillStyle = WCOLORS.teal;
    ctx.beginPath(); ctx.arc(bx + bw * 0.8, by + bh * 0.35, 1.5, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(bx + bw * 0.8, by + bh * 0.55, 1.5, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = WCOLORS.amber; ctx.lineWidth = 1;
    for (var j = 0; j < 3; j++) {
      var wy = by + bh * 0.3 + j * bh * 0.2;
      ctx.beginPath();
      for (var p = 0; p <= 5; p++) {
        var wx = bx + 6 + p * (bw * 0.45 / 5), wvy = wy + Math.sin(p * 1.5) * 2.5;
        if (p === 0) ctx.moveTo(wx, wvy); else ctx.lineTo(wx, wvy);
      }
      ctx.stroke();
    }
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('Microwave', cx, cy + s * 0.55 + 11);
  }

  function drawThermalCamera(cx, cy, s) {
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 1.5;
    ctx.strokeRect(cx - s * 0.8, cy - s * 0.5, s * 1.6, s * 1);
    ctx.beginPath(); ctx.arc(cx, cy, s * 0.32, 0, Math.PI * 2); ctx.stroke();
    var grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, s * 0.28);
    grad.addColorStop(0, 'rgba(255,255,255,0.9)');
    grad.addColorStop(0.3, 'rgba(255,200,0,0.7)');
    grad.addColorStop(0.7, 'rgba(255,60,0,0.5)');
    grad.addColorStop(1, 'rgba(160,0,0,0.3)');
    ctx.fillStyle = grad;
    ctx.beginPath(); ctx.arc(cx, cy, s * 0.28, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = WCOLORS.text; ctx.fillRect(cx + s * 0.45, cy - s * 0.5, s * 0.22, s * 0.2);
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('Thermal', cx, cy + s * 0.5 + 11);
  }

  function drawEye(cx, cy, s) {
    // Simple eye icon — the rainbow is already in the bar above
    var ey = cy;
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.moveTo(cx - s * 0.65, ey);
    ctx.quadraticCurveTo(cx, ey - s * 0.55, cx + s * 0.65, ey);
    ctx.quadraticCurveTo(cx, ey + s * 0.55, cx - s * 0.65, ey);
    ctx.stroke();
    // Iris — rainbow gradient
    var iGrad = ctx.createLinearGradient(cx - s * 0.22, ey, cx + s * 0.22, ey);
    iGrad.addColorStop(0, '#ff0000'); iGrad.addColorStop(0.17, '#ff8800');
    iGrad.addColorStop(0.33, '#ffcc00'); iGrad.addColorStop(0.5, '#00cc00');
    iGrad.addColorStop(0.67, '#0066ff'); iGrad.addColorStop(0.83, '#4400cc');
    iGrad.addColorStop(1, '#8800aa');
    ctx.fillStyle = iGrad;
    ctx.beginPath(); ctx.arc(cx, ey, s * 0.22, 0, Math.PI * 2); ctx.fill();
    // Pupil
    ctx.fillStyle = WCOLORS.text;
    ctx.beginPath(); ctx.arc(cx, ey, s * 0.09, 0, Math.PI * 2); ctx.fill();
    // Highlight
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.beginPath(); ctx.arc(cx + s * 0.06, ey - s * 0.06, s * 0.04, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('Human eye', cx, cy + s * 0.55 + 11);
  }

  function drawSunburn(cx, cy, s) {
    ctx.fillStyle = '#f59e0b';
    ctx.beginPath(); ctx.arc(cx, cy - s * 0.15, s * 0.28, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#f59e0b'; ctx.lineWidth = 1.5;
    for (var a = 0; a < 8; a++) {
      var ang = a * Math.PI / 4, r1 = s * 0.38, r2 = s * 0.55;
      ctx.beginPath();
      ctx.moveTo(cx + r1 * Math.cos(ang), cy - s * 0.15 + r1 * Math.sin(ang));
      ctx.lineTo(cx + r2 * Math.cos(ang), cy - s * 0.15 + r2 * Math.sin(ang));
      ctx.stroke();
    }
    ctx.fillStyle = '#7c3aed'; ctx.font = 'bold 9px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('UV', cx, cy + s * 0.5);
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui';
    ctx.fillText('Sunburn', cx, cy + s * 0.5 + 12);
  }

  function drawXrayHand(cx, cy, s) {
    // Simplified hand silhouette with bone lines
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 1;
    var px0 = cx - s * 0.4, py0 = cy - s * 0.4, pw = s * 0.8, ph = s * 0.9;
    ctx.beginPath();
    ctx.moveTo(px0 + 3, py0); ctx.lineTo(px0 + pw - 3, py0);
    ctx.quadraticCurveTo(px0 + pw, py0, px0 + pw, py0 + 3);
    ctx.lineTo(px0 + pw, py0 + ph - 3);
    ctx.quadraticCurveTo(px0 + pw, py0 + ph, px0 + pw - 3, py0 + ph);
    ctx.lineTo(px0 + 3, py0 + ph);
    ctx.quadraticCurveTo(px0, py0 + ph, px0, py0 + ph - 3);
    ctx.lineTo(px0, py0 + 3);
    ctx.quadraticCurveTo(px0, py0, px0 + 3, py0);
    ctx.stroke();
    ctx.strokeStyle = '#0ea5e9'; ctx.lineWidth = 2;
    var fxArr = [cx - s * 0.22, cx - s * 0.07, cx + s * 0.07, cx + s * 0.22];
    for (var fi = 0; fi < fxArr.length; fi++) {
      ctx.beginPath(); ctx.moveTo(fxArr[fi], py0 + ph * 0.1); ctx.lineTo(fxArr[fi], py0 + ph * 0.45); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(fxArr[fi], py0 + ph * 0.5); ctx.lineTo(fxArr[fi], py0 + ph * 0.75); ctx.stroke();
    }
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('X-ray', cx, cy + s * 0.5 + 11);
  }
  ...
  function drawNuclearSymbol(cx, cy, s) {
    ...
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('Nuclear', cx, cy + s * 0.6 + 11);
  }
  ...
  function draw() {
    ...
    // ---- Spectrum bar with rainbow in visible ----
    for (var bi = 0; bi < bands.length; bi++) {
      ...
      // Band name inside bar
      ctx.fillStyle = WCOLORS.text; ctx.font = '10px system-ui'; ctx.textAlign = 'center';
      var labelX = (x1 + x2) / 2;
      // For visible band, text won't read well on rainbow; skip label (the eye icon below is enough)
      if (b.name !== 'Visible') {
        ctx.fillText(b.name, labelX, barTop + barH / 2 + 3);
      }
    }
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1;
    ...
    // ---- Icon row ----
    // Space icons evenly across the full width so narrow bands don't cause overlap
    ctx.fillStyle = 'rgba(31,42,46,0.02)';
    ctx.fillRect(specLeft, iconRowTop, specW, iconRowH);

    var iconCy = iconRowTop + iconRowH * 0.4;
    var iconS = Math.min(13, iconRowH * 0.28);
    // Evenly spaced positions with connector lines to bands
    var iconNames = ['Gamma', 'X-ray', 'UV', 'Visible', 'Infrared', 'Microwave', 'Radio'];
    var nIcons = iconNames.length;
    for (var ii = 0; ii < nIcons; ii++) {
      var icx = specLeft + specW * (ii + 0.5) / nIcons;
      // Draw a thin line connecting icon to its band center
      var band = null;
      for (var jj = 0; jj < bands.length; jj++) {
        if (bands[jj].name === iconNames[ii]) { band = bands[jj]; break; }
      }
      if (band) {
        var bandCx = (logToX(band.logMin) + logToX(band.logMax)) / 2;
        ctx.strokeStyle = 'rgba(31,42,46,0.12)'; ctx.lineWidth = 0.5;
        ctx.beginPath(); ctx.moveTo(bandCx, lambdaAxisY + 4); ctx.lineTo(icx, iconRowTop + 4); ctx.stroke();
      }
      if (iconNames[ii] === 'Gamma')     drawNuclearSymbol(icx, iconCy, iconS);
      if (iconNames[ii] === 'X-ray')     drawXrayHand(icx, iconCy, iconS);
      if (iconNames[ii] === 'UV')        drawSunburn(icx, iconCy, iconS);
      if (iconNames[ii] === 'Visible')   drawEye(icx, iconCy, iconS);
      if (iconNames[ii] === 'Infrared')  drawThermalCamera(icx, iconCy, iconS);
      if (iconNames[ii] === 'Microwave') drawMicrowaveOven(icx, iconCy, iconS);
      if (iconNames[ii] === 'Radio')     drawRadioTower(icx, iconCy, iconS);
    }

    // ---- Marker ----
    var mx = logToX(markerLogLambda);
    if (mx >= specLeft && mx <= specRight) {
      // Dashed extensions above and below bar
      ...
      // Triangle below bar
      ctx.fillStyle = WCOLORS.text;
      ctx.beginPath(); ctx.moveTo(mx, barTop + barH + 2); ctx.lineTo(mx - 4, barTop + barH + 8); ctx.lineTo(mx + 4, barTop + barH + 8); ctx.closePath(); ctx.fill();
      ...
      if (nmVis >= 380 && nmVis <= 780) {
        ...
        ctx.beginPath(); ctx.arc(W * 0.88, infoY + 10, 6, 0, Math.PI * 2); ctx.fill();
...
function initMalusLaw() {
  ...
  let grabAngle = 0;   // mouse angle at grab time
  let grabTheta = 0;   // thetaDeg at grab time

  function mouseAngle(clientX, clientY) {
    const rect = canvas.getBoundingClientRect();
    const mx = clientX - rect.left - pol2X;
    const my = clientY - rect.top - cy;
    return Math.atan2(mx, -my); // angle from 12-o'clock, CW positive
  }
  ...
  function startDrag(clientX, clientY) {
    ...
    if (dist < polR + 25) {
      dragging = true;
      grabAngle = mouseAngle(clientX, clientY);
      grabTheta = thetaDeg;
    }
  }
  ...
  function updateDrag(clientX, clientY) {
    if (!dragging) return;
    const current = mouseAngle(clientX, clientY);
    let delta = (current - grabAngle) * 180 / Math.PI;
    thetaDeg = Math.round(Math.max(0, Math.min(90, grabTheta + delta)));
  }
  ...
  function draw() {
    ...
    if (!dragging) {
      ctx.fillStyle = WCOLORS.textDim; ctx.font = '9px system-ui'; ctx.textAlign = 'center';
      ctx.fillText('grab disc to rotate', pol2X, cy + polR + 28);
    }
...
// =========================================================================
// 3D MOVIE GLASSES (CHAPTER 14)
// =========================================================================

function init3DMovieGlasses() {
  const canvas = document.getElementById('scene-3d-movie-glasses');
  if (!canvas) return;
  const setup = wSetupCanvas(canvas);
  if (!setup) return;
  const { ctx, W, H } = setup;
...
function initThinFilmInterference() {
  ...
  const nFilm = 1.4;
  let t = 0;
  // 7 representative wavelengths spanning the visible spectrum
  const specWLs = [660, 610, 570, 530, 490, 455, 420];
  ...
  // Reflectance for wavelength wl at thickness d
  function reflectance(wl, d) {
    const delta = 2 * Math.PI * 2 * nFilm * d / wl + Math.PI;
    return Math.sin(delta / 2) ** 2;
  }
  ...
  // Draw a rainbow beam as parallel color stripes between two points.
  // alphaPerWL: function(wl) -> 0..1 brightness multiplier per wavelength, or null for uniform.
  // beamW: total width of rainbow spread.
  function drawRainbowBeam(x0, y0, x1, y1, beamW, alphaPerWL) {
    const dx = x1 - x0, dy = y1 - y0;
    const len = Math.sqrt(dx * dx + dy * dy);
    if (len < 1) return;
    const nx = -dy / len, ny = dx / len; // perpendicular
    const n = specWLs.length;
    const stripeW = beamW / n + 0.5;
    for (let i = 0; i < n; i++) {
      const off = ((i + 0.5) / n - 0.5) * beamW;
      const wl = specWLs[i];
      const [cr, cg, cb] = wavelengthToRGB(wl);
      const a = alphaPerWL ? alphaPerWL(wl) : 1;
      if (a < 0.01) continue;
      ctx.strokeStyle = `rgba(${cr},${cg},${cb},${(a * 0.6).toFixed(2)})`;
  ...
  function draw() {
    ...
    const thetaR = Math.asin(Math.sin(thetaI) / nFilm);
    const beamW = 16;
    ...
    // R1 exits from A upward-right
    const r1EndX = Ax + inLen * Math.tan(thetaI);
    const r1EndY = 16;
    // R2 exits from C upward-right (parallel to R1)
    const r2EndX = Cx + inLen * Math.tan(thetaI);
    const r2EndY = 16;

    // --- 1. Incoming white light: full rainbow, all colors at full brightness ---
    drawRainbowBeam(inStartX, inStartY, Ax, Ay, beamW, null);

    // Animated wave fronts in incoming beam
    ctx.save();
    const inDx = Ax - inStartX, inDy = Ay - inStartY;
    const inLn = Math.sqrt(inDx * inDx + inDy * inDy);
    const inNx = -inDy / inLn, inNy = inDx / inLn;
    ctx.beginPath();
    ctx.moveTo(inStartX + inNx * beamW * 0.7, inStartY + inNy * beamW * 0.7);
    ctx.lineTo(Ax + inNx * beamW * 0.7, Ay + inNy * beamW * 0.7);
    ctx.lineTo(Ax - inNx * beamW * 0.7, Ay - inNy * beamW * 0.7);
    ctx.lineTo(inStartX - inNx * beamW * 0.7, inStartY - inNy * beamW * 0.7);
    ctx.closePath(); ctx.clip();
    ctx.strokeStyle = 'rgba(255,255,255,0.25)'; ctx.lineWidth = 1;
    for (let s = (t * 40) % 12; s < inLn; s += 12) {
      const frac = s / inLn;
      const px = inStartX + inDx * frac, py = inStartY + inDy * frac;
      ctx.beginPath();
      ctx.moveTo(px + inNx * beamW * 0.6, py + inNy * beamW * 0.6);
      ctx.lineTo(px - inNx * beamW * 0.6, py - inNy * beamW * 0.6);
      ctx.stroke();
    }
    ctx.restore();
    ctx.fillStyle = WCOLORS.text; ctx.font = '10px system-ui'; ctx.textAlign = 'center';
    ...
    drawRainbowBeam(Bx, By, Cx, Cy, beamW * 0.55, null);
    ...
    ctx.fillText('R\u2081 gets \u03c0 phase shift \u2192 some colors cancel, some reinforce', filmL, H - 4);
...
function initPrismDispersion() {
  ...
  if (!apexSlider) {
    ...
    if (parent) {
      ...
      controls.className = 'scene-controls';
      controls.innerHTML =
        '<label>Apex angle: <input type="range" id="prism-apex" min="30" max="75" step="1" value="60"><span class="scene-val" id="prism-apex-val">60°</span></label>' +
        '<label>Incidence angle: <input type="range" id="prism-incidence" min="20" max="80" step="1" value="45"><span class="scene-val" id="prism-incidence-val">45°</span></label>' +
        '<label>Index n<sub>0</sub>: <input type="range" id="prism-n0" min="1.30" max="1.80" step="0.01" value="1.52"><span class="scene-val" id="prism-n0-val">1.52</span></label>';
      parent.appendChild(controls);
  ...
  function draw() {
    ...
    const n0 = parseFloat(n0Slider?.value || 1.52);
    const apex = apexDeg * Math.PI / 180;
    const thetaI1 = incDeg * Math.PI / 180;

    document.getElementById('prism-apex-val')?.replaceChildren(document.createTextNode(apexDeg + '°'));
    document.getElementById('prism-incidence-val')?.replaceChildren(document.createTextNode(incDeg + '°'));
...
function initInterferometerResolution() {
  ...
  // --- Slider hit test ---
  function hitTest(mx, my) {
    if (mx >= slX - 5 && mx <= slX + slW + 5) {
      if (Math.abs(my - sl1Y) < 10) return 'sep';
      if (Math.abs(my - sl2Y) < 10) return 'base';
      if (Math.abs(my - sl3Y) < 10) return 'nant';
    }
    return null;
  }
  ...
  function drawObservedImage() {
    ...
    for (var py = 0; py < imgH; py++) {
      for (var px = 0; px < colW; px++) {
        ...
        var I1 = beam(Math.sqrt(r1sq) / scale);
        var I2 = beam(Math.sqrt(r2sq) / scale);
        var total = I1 + I2;
  ...
  function drawProfile() {
    ...
    ctx.fillText('Intensity Profile', pL + pW / 2, pT - 4);

    var thetaRange = Math.max(starSep * 2, resAngle * 3, 0.03);
    var yScale = (pH - 8) * 0.43;

    // Beam pattern alone (dashed, shows sidelobes)
    ctx.strokeStyle = 'rgba(15,118,110,0.3)'; ctx.lineWidth = 1; ctx.setLineDash([3, 3]);
    ctx.beginPath();
    for (var px = 0; px <= pW; px++) {
      var theta = (px / pW - 0.5) * 2 * thetaRange;
      var py2 = pT + pH - 3 - beam(theta) * yScale;
      if (px === 0) ctx.moveTo(pL + px, py2); else ctx.lineTo(pL + px, py2);
    }
    ctx.stroke(); ctx.setLineDash([]);

    // Observed: beam from each star summed (solid bold)
    ctx.strokeStyle = WCOLORS.text; ctx.lineWidth = 2;
    ctx.beginPath();
    for (var px = 0; px <= pW; px++) {
      var theta = (px / pW - 0.5) * 2 * thetaRange;
      var I = beam(theta + starSep / 2) + beam(theta - starSep / 2);
  ...
  function drawSliders() {
    ...
    ctx.fillStyle = WCOLORS.text; ctx.font = '10px system-ui'; ctx.textAlign = 'left';
    fillTextSub(ctx, '\u03B8_{sep} = ' + (starSep * 180 / Math.PI).toFixed(1) + '\u00B0', slX + slW + 10, sl1Y + 4);
  ...
  function tick() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let wf = 0; wf < 5; wf++) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(gratingX, sy, r, -Math.PI / 2, Math.PI / 2);
        ctx.stroke();
      }
    }
    ctx.restore();
    ...
    // Vertical axis (sin θ)
    ctx.beginPath(); ctx.moveTo(plotL, plotT); ctx.lineTo(plotL, plotB); ctx.stroke();
    // Horizontal axis (I/I₀) at sin θ = 0
    ctx.beginPath(); ctx.moveTo(plotL, gratingCY); ctx.lineTo(plotR, gratingCY); ctx.stroke();

    // sin θ tick marks and grid lines
    ctx.fillStyle = WCOLORS.textDim; ctx.font = '9px system-ui'; ctx.textAlign = 'right';
    for (let v = -1; v <= 1; v += 0.5) {
      const py = gratingCY + v * (pH / 2);
    ...
    ctx.save();
    ctx.fillStyle = WCOLORS.text; ctx.font = '9px system-ui'; ctx.textAlign = 'center';
    ctx.translate(plotL - 18, gratingCY);
    ctx.rotate(-Math.PI / 2);
    ...
    ctx.beginPath();
    for (let py = 0; py <= nPlotSamples; py++) {
      const norm = plotIntensities[py] / Math.max(plotMaxI, 0.01);
      const px = plotL + norm * pW * 0.9;
    ...
    // Mark principal maxima with order labels
    ctx.fillStyle = WCOLORS.amber; ctx.font = '9px system-ui'; ctx.textAlign = 'left';
    for (let m = -4; m <= 4; m++) {
      const sinTheta = m / dOverLambda;
      if (Math.abs(sinTheta) > 0.98) continue;
      const yy = gratingCY - sinTheta * (pH / 2);
      if (yy < plotT + 3 || yy > plotB - 3) continue;
      const px = plotL + pW * 0.9;
      ctx.beginPath(); ctx.arc(px, yy, 3, 0, Math.PI * 2); ctx.fill();
      ctx.fillText('m=' + m, px + 5, yy + 3);
    }
...
function initSingleSlitDiffraction() {
  ...
  function tick() {
    ...
    // Interior gap is larger than the slit
    const gapH = areaH * 0.45;
    const gapTop = slitCY - gapH / 2;
    const gapBot = slitCY + gapH / 2;

    // The actual slit is smaller
    const slitTop = slitCY - slitOpenH / 2;
    const slitBot = slitCY + slitOpenH / 2;

    // Light glow visible through interior gap (pulses with emission)
    var glowAlpha = 0.08 + glowStrength * 0.5;
    const grad = ctx.createRadialGradient(boxL + boxW * 0.4, slitCY, 2, boxL + boxW * 0.4, slitCY, gapH * 0.6);
    grad.addColorStop(0, 'rgba(15,118,110,' + (glowAlpha * 0.9) + ')');
    grad.addColorStop(0.6, 'rgba(15,118,110,' + (glowAlpha * 0.4) + ')');
    grad.addColorStop(1, 'rgba(15,118,110,0)');
    ctx.save();
    ctx.beginPath();
    ctx.rect(boxL, gapTop, boxW, gapH);
    ctx.clip();
    ctx.fillStyle = grad;
    ctx.fillRect(boxL, gapTop - gapH, boxW, gapH * 3);
    ctx.restore();

    // Box walls — top and bottom dark blocks (same as original)
    ctx.fillStyle = WCOLORS.axis;
    ctx.fillRect(boxL, areaTop, boxW, gapTop - areaTop);
    ctx.fillRect(boxL, gapBot, boxW, areaBot - gapBot);
    // Back wall
    ...
    // Front wall pieces that narrow the exit to the slit
    ctx.fillRect(boxR - 3, gapTop, 3, slitTop - gapTop);
    ctx.fillRect(boxR - 3, slitBot, 3, gapBot - slitBot);
...
// =========================================================================
// 15. Fourier Optics — Aperture → Diffraction Pattern
// =========================================================================
function initFourierOpticsDemo() {
  ...
  // Aperture: N samples over x in [-5, 5], values 0 to 1
  const N = 256;
  const aperture = new Float32Array(N);
  let time = 0;
  let patternDirty = true;

  // Diffraction pattern: nPat bins over sin(theta) in [-1, 1]
  const nPat = 400;
  const pattern = new Float32Array(nPat);
  let patMax = 0;
  const kScale = 10; // controls number of fringes: larger = more fringes

  // Mode: 0 = Square, 1 = Gaussian, 2 = Draw
  let mode = 0;
  let slitWidth = 2.0;  // half-width for square (in x-units, max ~4.5)
  let gaussSigma = 1.5;
  ...
  // Aperture panel (narrow)
  const apL = 12, apR = 62, apW = apR - apL;

  // Barrier
  const barX = 68, barW = 4;

  // Screen (FT image)
  const scL = W * 0.52, scR = W - 12, scW = Math.round(scR - scL);

  // Offscreen buffer for screen image
  const scBuf = document.createElement('canvas');
  scBuf.width = scW; scBuf.height = nPat;
  ...
  function paintAt(mx, my) {
    ...
    var bR = 3;
    for (var di = -bR; di <= bR; di++) {
      var ii = idx + di;
      if (ii < 0 || ii >= N) continue;
      var w = 1 - Math.abs(di) / (bR + 1);
  ...
  function paintInterp(mx, my) {
    if (lastDY >= 0) {
      var dy = my - lastDY, dx = mx - lastDX;
      var steps = Math.max(1, Math.ceil(Math.abs(dy) / 2));
      for (var s = 0; s <= steps; s++) {
        var t = s / steps;
        paintAt(lastDX + dx * t, lastDY + dy * t);
      }
    } else { paintAt(mx, my); }
    lastDX = mx; lastDY = my;
  }
  ...
  function onDown(mx, my) {
    ...
    if (my > tabY - 3 && my < tabY + tabH + 3) {
      for (var i = 0; i < nTabs; i++) {
        ...
        if (mx > tx && mx < tx + tabW) {
          if (mode !== i) { mode = i; if (mode === 2) { aperture.fill(0); patternDirty = true; } else buildAperture(); }
          return;
        }
      }
    }
    // Slider
    if (mode < 2 && Math.abs(my - sliderY) < 14) {
      draggingSlider = true;
      onSlider(mx); return;
    }
    // Draw
    if (mode === 2 && mx >= apL - 6 && mx <= apR + 6 && my >= topY && my <= botY) {
      drawing = true; lastDX = -1; lastDY = -1;
      paintInterp(mx, my);
    }
  }

  function onSlider(mx) {
    var t = Math.max(0, Math.min(1, (mx - sliderX) / sliderW));
    if (mode === 0) slitWidth = 0.4 + t * 4.1;
    else gaussSigma = 0.3 + t * 3.7;
    buildAperture();
  }

  canvas.addEventListener('mousedown', function(e) { var p = xy(e); onDown(p.x, p.y); });
  canvas.addEventListener('mousemove', function(e) {
    var p = xy(e);
    if (draggingSlider) { onSlider(p.x); return; }
    if (drawing && mode === 2) paintInterp(p.x, p.y);
  });
  canvas.addEventListener('mouseup', function() { draggingSlider = false; drawing = false; lastDX = -1; lastDY = -1; });
  canvas.addEventListener('mouseleave', function() { draggingSlider = false; drawing = false; lastDX = -1; lastDY = -1; });
  canvas.addEventListener('touchstart', function(e) {
    e.preventDefault(); var r = canvas.getBoundingClientRect();
  ...
  canvas.addEventListener('touchmove', function(e) {
    ...
    if (draggingSlider) { onSlider(mx); return; }
    if (drawing && mode === 2) paintInterp(mx, my);
  }, { passive: false });
  canvas.addEventListener('touchend', function() { draggingSlider = false; drawing = false; lastDX = -1; lastDY = -1; });

  // ---- Compute diffraction pattern ----
  // Evaluates |FT|^2 at nPat points of sin(theta) from -1 to +1
  function recomputePattern() {
    patMax = 0;
    for (var j = 0; j < nPat; j++) {
      var sinTh = -1 + 2 * j / (nPat - 1);
      var re = 0, im = 0;
      for (var n = 0; n < N; n++) {
        var xn = (n / N - 0.5) * 10; // position in [-5, 5]
        var ph = kScale * xn * sinTh;
        re += aperture[n] * Math.cos(ph);
        im += aperture[n] * Math.sin(ph);
      }
      var I = (re * re + im * im);
      pattern[j] = I;
      if (I > patMax) patMax = I;
    }
    // Render 2D screen to offscreen buffer
    var imgData = scCtx.createImageData(scW, nPat);
    var d = imgData.data;
    var gamma = 0.45;
    var bandHW = scW * 0.45;
    for (var py = 0; py < nPat; py++) {
      var Inorm = pattern[py] / Math.max(patMax, 0.001);
      var base = Inorm > 0.002 ? Math.pow(Inorm, gamma) * 255 : 0;
      for (var px = 0; px < scW; px++) {
        var dx = (px - scW / 2) / bandHW;
        var val = Math.round(base * Math.exp(-dx * dx * 1.5));
        var off = (py * scW + px) * 4;
  ...
  function tick() {
    ...
    ctx.beginPath(); ctx.moveTo(apL, topY);
    for (var i = 0; i < N; i++) ctx.lineTo(apL + aperture[i] * apW, topY + (i / N) * areaH);
    ctx.lineTo(apL, botY); ctx.closePath(); ctx.fill();
    ...
    for (var i = 0; i < N; i++) {
      ...
      if (i === 0) ctx.moveTo(apL + aperture[i] * apW, yy);
      else ctx.lineTo(apL + aperture[i] * apW, yy);
    }
    ctx.stroke();
    ...
    // Label
    ctx.fillStyle = WCOLORS.text; ctx.font = 'bold 10px system-ui'; ctx.textAlign = 'center';
    fillTextSub(ctx, 'T(x)', (apL + apR) / 2, topY - 7);

    // ===== Barrier =====
    ctx.fillStyle = WCOLORS.axis;
    // Draw as continuous wall with gaps for transparent regions
    var prevOpaque = true;
    for (var i = 0; i <= N; i++) {
      var isOpaque = i === N || aperture[Math.min(i, N - 1)] < 0.1;
      if (isOpaque) {
        var yy = topY + (i / N) * areaH;
        ctx.fillRect(barX - barW / 2, yy, barW, Math.ceil(areaH / N) + 1);
      }
    }

    // ===== Incoming plane waves =====
    var waveL2 = apR + 3, waveR2 = barX - barW / 2 - 1;
    var span = waveR2 - waveL2;
    if (span > 4) {
      ctx.lineWidth = 1.5;
      for (var wf = 0; wf < 8; wf++) {
        var x = ((time * 12 - wf * lambda) % span);
        if (x < 0) x += span;
        x += waveL2;
        if (x > waveL2 + 1 && x < waveR2 - 1) {
          ctx.strokeStyle = 'rgba(15,118,110,' + (0.08 + 0.22 * ((x - waveL2) / span)).toFixed(3) + ')';
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(barX + barW / 2, topY, scL - barX - barW / 2 - 4, areaH);
    ctx.clip();
    var nSrc = 16;
    for (var s = 0; s < nSrc; s++) {
      ...
      for (var wf = 0; wf < 5; wf++) {
        ...
        if (r < 3) continue;
        var al = Math.max(0, 0.12 * amp * (1 - r / (W * 0.45)));
        if (al < 0.005) continue;
        ctx.strokeStyle = 'rgba(15,118,110,' + al.toFixed(3) + ')';
        ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(barX, sy, r, -Math.PI / 2, Math.PI / 2); ctx.stroke();
      }
    }
    ctx.restore();
    ...
    ctx.fillText('FT \u2192', (barX + scL) / 2, topY - 7);

    // ===== Screen image =====
    ctx.fillStyle = WCOLORS.text; ctx.font = 'bold 10px system-ui'; ctx.textAlign = 'center';
    fillTextSub(ctx, '|FT|^{2}', (scL + scR) / 2, topY - 7);

    // Blit offscreen buffer
    ctx.drawImage(scBuf, 0, 0, scW, nPat, scL, topY, scW, areaH);
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1;
    ctx.strokeRect(scL, topY, scW, areaH);

    // 1D intensity curve overlay (amber, on left portion of screen)
    ctx.strokeStyle = WCOLORS.amber; ctx.lineWidth = 2;
    ctx.beginPath();
    for (var j = 0; j <= areaH; j++) {
      var k = Math.floor(j / areaH * (nPat - 1));
      var Iv = pattern[k] / Math.max(patMax, 0.001);
      var px = scL + 4 + Iv * scW * 0.35;
      if (j === 0) ctx.moveTo(px, topY); else ctx.lineTo(px, topY + j);
    }
    ctx.stroke();

    // sin(theta) labels on right edge
    ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.font = '8px system-ui'; ctx.textAlign = 'right';
    ctx.fillText('+1', scR - 3, topY + 8);
    ctx.fillText('0', scR - 3, midY + 3);
    ctx.fillText('\u20131', scR - 3, botY - 2);
    ctx.save();
    ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '8px system-ui'; ctx.textAlign = 'center';
    ctx.translate(scR - 6, midY);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('sin \u03B8', 0, 0);
    ctx.restore();
    ...
    if (mode < 2) {
      ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(sliderX, sliderY); ctx.lineTo(sliderX + sliderW, sliderY); ctx.stroke();
      var t = mode === 0 ? (slitWidth - 0.4) / 4.1 : (gaussSigma - 0.3) / 3.7;
      ctx.beginPath(); ctx.arc(sliderX + sliderW * t, sliderY, 6, 0, Math.PI * 2);
      ctx.fillStyle = WCOLORS.teal; ctx.fill();
      ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = WCOLORS.text; ctx.font = '10px system-ui'; ctx.textAlign = 'left';
      if (mode === 0) ctx.fillText('width = ' + (slitWidth * 2).toFixed(1), sliderX + sliderW + 14, sliderY + 4);
      else ctx.fillText('\u03C3 = ' + gaussSigma.toFixed(1), sliderX + sliderW + 14, sliderY + 4);
    } else {
      ctx.fillStyle = WCOLORS.textDim; ctx.font = '10px system-ui'; ctx.textAlign = 'left';
      ctx.fillText('Drag on T(x) to draw your aperture', sliderX, sliderY + 4);
    }
...
// =========================================================================
// 17. Double Slit Electron Buildup
// =========================================================================
function initDoubleSlitPhotonBuildup() {
  ...
  var slitSepNorm = 0.5;
  var slitWidthNorm = 0.4;

  function getSlitSep() { return regionH * (0.10 + slitSepNorm * 0.38); }
  function getSlitHalfW() { return regionH * (0.012 + slitWidthNorm * 0.055); }
  ...
  function prob(yNorm, dP, aP, block) {
    ...
    if (block === 1 || block === 2) {
      ...
      return Math.abs(sa2) < 1e-6 ? 1 : Math.pow(Math.sin(sa2) / sa2, 2);
    }
  ...
  function drawGun() {
    ...
    grad.addColorStop(0.7, '#718096'); grad.addColorStop(1, '#4a5568');
    ctx.fillStyle = grad;
  ...
  function drawBarrier() {
    ...
    ctx.fillRect(barrierX - barrierW/2, s2 + shw, barrierW, regionB + 5 - s2 - shw);
    // 3D edges
  ...
  function drawSlider(label, y, val01) {
    ...
    ctx.beginPath(); ctx.arc(sldrL + sldrW * val01, y, 5, 0, Math.PI * 2);
    ctx.fillStyle = WCOLORS.blue; ctx.fill();
    ctx.strokeStyle = WCOLORS.axis; ctx.lineWidth = 1; ctx.stroke();
  }
